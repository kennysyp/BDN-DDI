...
def get_mol_edge_list_and_feat_mtx(mol_graph):
    ...
    return new_edge_index, n_features,edge_feats
...
def create_graph_data(id):
    ...
    edge_feature = MOL_EDGE_LIST_FEAT_MTX[id][2]
...
class BipartiteData(Data):
    def __init__(self, edge_index=None, x_s=None, x_t=None,h=None,t=None):
        super().__init__()
...
class DrugDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        for h, t, r in batch:
            ...
            pos_b_graph = self._create_b_graph(get_bipartite_graph(h_graph, t_graph), h_data.x, t_data.x,h,t)
        ...
        for i in range(len(neg_h_test)):
            ...
            neg_b_graph = self._create_b_graph(get_bipartite_graph(neg_h_graph, neg_t_graph), neg_h_samples[i].x,
                                               neg_t_samples[i].x,neg_h_test[i],neg_t_test[i])
    ...
    def _create_b_graph(self, edge_index, x_s, x_t,h,t):
        return BipartiteData(edge_index, x_s, x_t,h,t)
